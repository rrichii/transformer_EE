...
// Print the contents of the dictionary in the order they were read
    ...
    outfile << "\"Event_Index\",\"Nu_PDG\",\"Nu_Energy\",\"Nu_Mom_X\",\"Nu_Mom_Y\",\"Nu_Mom_Z\",\"Nu_CosTheta\",\"Nu_Theta\",\"Nu_Phi\",\"Nu_Baseline\",\"Lepton_PDG\",\"Lepton_Energy\",\"Lepton_MomX\",\"Lepton_MomY\",\"Lepton_MomZ\",\"Lepton_CosTheta\",\"Lepton_Theta\",\"Final_State_Particles_PDG\",\"Final_State_Particles_Mass\",\"Final_State_Particles_Energy\",\"Final_State_Particles_Momentum_X\",\"Final_State_Particles_Momentum_Y\",\"Final_State_Particles_Momentum_Z\",\"Final_State_Particles_CosTheta\",\"Final_State_Particles_Theta\",\"tot_fKE\",\"p_tot\",\"P_miss\"\n";
    ...
    {   //Initializing multiplicity variables
        ...
        (
            ...
            // //std::cout <<" i issss "<<i<< " and j isssss " << j << " and status is " << tStdHepStatus[j] <<" and pdg " << tStdHepPdg[j]<< std::endl;
                ...
                if(tStdHepStatus[j] == 1)
                {
                    ...
                    if(tStdHepPdg[j] == 11 || tStdHepPdg[j] == 13 || tStdHepPdg[j] == 15 || tStdHepPdg[j] == -11 || tStdHepPdg[j] == -13 || tStdHepPdg[j] == -15)
                    {
                        ...
                        outfile << std::setprecision(6) << tStdHepPdg[j] << "\",\"" << fKE << "\",\"" << tStdHepP4[4*j] << "\",\"" << tStdHepP4[4*j+1] << "\",\"" << tStdHepP4[4*j+2] << "\",\"" << tStdHepP4[4*j+1]/fAbsoluteParticleMomentum << "\",\"" << (180./PI)*acos(tStdHepP4[4*j+1]/fAbsoluteParticleMomentum) << "\",\"";
                    ...
                    if (tStdHepPdg[j] == 2212 || tStdHepPdg[j] == 211 || tStdHepPdg[j] == -211 || tStdHepPdg[j] == 111 || tStdHepPdg[j] == 321 || tStdHepPdg[j] == -321 || tStdHepPdg[j] == 130 || tStdHepPdg[j] == 310)
                    {                       
            ...
            {
                if ( j <  pdgs.size()-1 ) { outfile << setprecision(6) << pdgs[j] << ",";}
                if ( j == pdgs.size()-1 ) { outfile << setprecision(6) << pdgs[j] << "\"";}
            } 
            }   
            outfile << ",\"";
            ...
            if (masses.empty())       masses.push_back(0);
            if (energies.empty())     energies.push_back(0);
            if (pxs.empty())          pxs.push_back(0);
            if (pys.empty())          pys.push_back(0);
            if (pzs.empty())          pzs.push_back(0);
            if (costheta_arr.empty()) costheta_arr.push_back(0);
            if (theta_arr.empty())    theta_arr.push_back(0);
