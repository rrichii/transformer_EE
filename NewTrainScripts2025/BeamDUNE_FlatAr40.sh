...
readarray -t Json_files < BeamNOvAND_NatJson.txt
readarray -t CSV_files < CSV.txt
...
for json_file in "${Json_files[@]}"; do
    ...
    for csv_file in "${CSV_files[@]}"; do
        ...
        if [[ "$json_base" == *"$csv_base"* ]]; then
            ...
            out_dir="/exp/dune/data/users/${USER}/MLProject/Training_Samples/Beam_Like/Natural_Spectra/NOvAND/${json_base}"
            ...
            python3 train_wide.py \
                ...
                --wandb-id "${short_base}_d256_n16_do0p3" \
                --dataframe-type polars
        fi
    done
done
